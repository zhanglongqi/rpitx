...
int SetupGpioClock(uint32_t SymbolRate, double TuningFrequency) {
    ...
    char OutputPower = 7;
    pad_gpios_reg[PADS_GPIO_0] =
            0x5a000000 + (OutputPower & 0x7) + (1 << 4) + (0 << 3); // Set output power for I/Q GPIO18/GPIO19
    ...
    FreqFractionnalPCM =
            4096.0 * (((double) PllFreq1GHZ / (SymbolRate * NbStepPCM/**PwmNumberStep*/)) - FreqDividerPCM);
...
//@1 = CBS_MAIN+1	Length*27ns			
    cbp->info = BCM2708_DMA_WAIT_RESP |
                /*BCM2708_DMA_NO_WIDE_BURSTS| BCM2708_DMA_WAIT_RESP|*//*(0x8<< 21) |*/ BCM2708_DMA_SRC_INC;
    cbp->src = mem_virt_to_phys(
            &ctl->SharedFrequencyTab[0]);//mem_virt_to_phys(&ctl->SharedFrequencyTab[0]);//mem_virt_to_phys(&ctl->SharedFrequency1);
    cbp->dst = phys_pwm_clock_div_addr;
...
inline void FrequencyAmplitudeToRegister(double TuneFrequency, uint32_t Amplitude, int NoSample,
                                         uint32_t WaitNanoSecond, uint32_t SampleRate, char NoUsePWMF, int debug) {
    ...
    */
    ...
    if (ShowInfo == 1) {
        printf("WaitNano=%d F1=%f TuneFrequency %f F2=%f Initial Resolution(Hz)=%f ResolutionPWMF %f NbStep=%d DELAYStep=%d\n",
               WaitNanoSecond, f1, TuneFrequency, f2, FreqStep, FreqStep / (PwmNumberStep), PwmNumberStep,
               (PWMF_MARGIN + FREQ_DELAY_TIME) / FREQ_MINI_TIMING);
        ShowInfo = 0;
    }


    static int DebugStep = 71;
    double fPWMFrequency = ((FreqTuningUp - TuneFrequency) * 1.0 * (double) (PwmNumberStep) /
                            FreqStep); // Give NbStep of F2
...
//if((CompteurDebug%200)==0) printf("PwmNumberStep =%d TuneFrequency %f : FreqTuning %f FreqStep %f PwmFreqStep %f fPWMFrequency %f PWMFrequency %d f1 %f f2 %f %x %x\n",PwmNumberStep,TuneFrequency,FreqTuning,FreqStep,FreqStep/PwmNumberStep,fPWMFrequency,PWMFrequency,f1,f2,RegisterF1,RegisterF2);
    ...
    */
    ...
    //printf("%d\n",IntAmplitude);
    ctl->sample[NoSample].Amplitude1 =
            0x5a000000 + (IntAmplitude & 0x7) + (1 << 4) + (0 << 3); // Set output power for I/Q GPIO18/GPIO19;//TEST +1
...
// -----------------------------------------------------------------
    ...
    for (; ;) {
        ...
        if ((Init == 1) && (free_slots < DmaSampleBurstSize /*NUM_SAMPLES/8*/)) {
            ...
            //Start Main DMA
            dma_reg[DMA_CS + DMA_CHANNEL *
                             0x40] = 0x10FF0001;    // go, mid priority, wait for outstanding writes :7 Seems Max Priority
        ...
        if ((free_slots >= DmaSampleBurstSize)) {
            ...
            if (Mode == MODE_IQ) {
                ...
                for (i = 0; i < DmaSampleBurstSize; i++) {
                    ...
                    ampf = (fabs(ampf) < 1.0f / A) ? A * fabs(ampf) / (1.0f + ln(A)) : (1.0f + ln(A * fabs(ampf))) /
                                                                                       (1.0f + ln(A)); //compand
                    ...
                    */
                    ...
                    FrequencyAmplitudeToRegister(
                            (GlobalTuningFrequency - OffsetModulation +/*(CompteSample/480)*/+df / HarmonicNumber) /
                            HarmonicNumber, amp, last_sample++, 0, SampleRate, NoUsePwmFrequency, CompteSample % 2);
            ...
            if (Mode == MODE_IQ_FLOAT) {
                ...
                for (i = 0; i < DmaSampleBurstSize; i++) {
                    ...
                    */
                    ...
                    FrequencyAmplitudeToRegister(
                            (GlobalTuningFrequency - OffsetModulation + df / HarmonicNumber) / HarmonicNumber, amp,
                            last_sample++, 0, SampleRate, NoUsePwmFrequency, CompteSample % 2);
...
#define MAX_DELAY_WAIT 25000
                ...
                for (i = 0; i < DmaSampleBurstSize; i++) {
                    ...
                    if (Mode == MODE_RF) {
                        if (SampleRf.Frequency == 0.0) {
                            ...
                            amp = 32767;
                        FrequencyAmplitudeToRegister(
                                (SampleRf.Frequency / HarmonicNumber + GlobalTuningFrequency) / HarmonicNumber, amp,
                                last_sample++, WaitSample, 0, NoUsePwmFrequency, debug);
                    }
                    if (Mode == MODE_RFA)
                        FrequencyAmplitudeToRegister((GlobalTuningFrequency) / HarmonicNumber, SampleRf.Frequency,
                                                     last_sample++, WaitSample, 0, NoUsePwmFrequency, debug);
            ...
            if (Mode == MODE_VFO) {
                ...
                for (i = 0; i < DmaSampleBurstSize; i++) {
                    ...
                    FrequencyAmplitudeToRegister(GlobalTuningFrequency / HarmonicNumber/*+(CompteSample*0.1)*/,
                                                 OutputPower, last_sample++, 25000, 0, NoUsePwmFrequency, debug);
                    free_slots--;
